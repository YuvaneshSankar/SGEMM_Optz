...
__global__ void coalescing_kernel(float *A, float *B, float *C, int m, int n, int p){
...
int main(){
    ...
    //launch kernel
    dim3 blockDim(BLOCK_SIZE*BLOCK_SIZE); //1024 changed from 32,32 to 32*32
    dim3 gridDim((P + BLOCK_SIZE - 1) / BLOCK_SIZE, (M + BLOCK_SIZE - 1) / BLOCK_SIZE);

    //warm up runs

    for(int i=0;i<3;i++){
        coalescing_kernel<<<gridDim,blockDim>>>(d_A,d_B,d_C,M,N,P);

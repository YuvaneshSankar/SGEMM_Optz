...
__global__ void kernel(float * A , float * B , float * C , int m , int n , int p){
    int col= blockDim.x*blockIdx.x + threadIdx.x;
    int base_row= (blockDim.y*blockIdx.y + threadIdx.y )* COARSENING_FACTOR;
...
int main(){
    ...
    for(int i=0;i<3;i++){
        kernel<<<gridDim,blockDim>>>(d_A,d_B,d_C,M,N,P);
